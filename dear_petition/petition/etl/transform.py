    ...
    with transaction.atomic():
        ...
        for batch in batches:
            for record in batch.records.iterator():
                ...
                export_record_dataset = record_resource.export(
                    queryset=pm.CIPRSRecord.objects.filter(id=record.id)
                )
                import_record_dataset = tablib.Dataset()
                import_record_dataset.headers = [
                    col.column_name for col in record_resource.get_import_fields()
                ]
                import_record_dataset.append(
                    [new_batch.id]
                    + [value if value else None for value in export_record_dataset[0]]
                )
                ...
                for offense in record.offenses.all():
                    export_offense_dataset = offense_resource.export(
                        queryset=pm.Offense.objects.filter(id=offense.id)
                    )
                    import_offense_dataset = tablib.Dataset()
                    import_offense_dataset.headers = [
                        col.column_name for col in offense_resource.get_import_fields()
                    ]
                    import_offense_dataset.append(
                        [record_id]
                        + [value if value else None for value in export_offense_dataset[0]]
                    )
                    offense_resource.import_data(import_offense_dataset)
                    offense_id = offense_resource.saved_instance_ids[-1]

                    export_offense_record_dataset = offense_record_resource.export(
                        queryset=pm.OffenseRecord.objects.filter(offense_id=offense.id)
                    )
                    import_offense_record_dataset = tablib.Dataset()
                    import_offense_record_dataset.headers = [
                        col.column_name for col in offense_record_resource.get_import_fields()
                    ]
                    for row in export_offense_record_dataset.dict:
                        import_offense_record_dataset.append(
                            [offense_id] + [value if value else None for value in row.values()]
                        )
