...
def test_generated_petition_admin():
    # test constants
    READ_ONLY_FIELDS = ["username", "batch_id", "form_type", "number_of_charges", "county", "jurisdiction", "race",
                        "sex", "age"]
    ...
    assert(list(gp_admin.get_fieldsets(request)) == [(None, {"fields": READ_WRITE_FIELDS + READ_ONLY_FIELDS})])
