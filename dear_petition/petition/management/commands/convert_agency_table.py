...
def convert_contacts_to_agency_objects(current_cls, target_cls):
    ...
    for agency_contact in agency_contacts.values():
        with transaction.atomic():
            ...
            if re.search(
                models.AGENCY_SHERRIFF_OFFICE_PATTERN, agency.name, re.IGNORECASE
            ) or re.search(models.AGENCY_SHERRIFF_DEPARTMENT_PATTERN, agency.name, re.IGNORECASE):
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            "--convert_contacts",
            action="store_true",
            help="Convert existing agencies in Contact table to Agency table",
        )
        parser.add_argument(
            "--convert_agencies",
            action="store_true",
            help="Convert existing agencies in Agency table to Contact table",
        )
    ...
    def handle(self, *args, **options):
        if options["convert_contacts"]:
            ...
            raise CommandError(
                "Did not provide one of `convert_contacts` or `convert_agencies` arguments"
            )
