...
class MultiModelResource(resources.ModelResource):
        ...
        def export_field(self, field, obj):
            ...
            if isinstance(self._meta.model._meta.get_field(field.attribute), BooleanField):
                field.dropdown = DataValidation(type="list", formula1=f'"True,False"', showDropDown=True)
        ...
        def export_excel(self, data, dataset: ExcelDataset, color=None, **kwargs):   
            ...
            dataset.append(headers, fields, num_indent=self.NUM_INDENT, is_header=True, bold=True, color=color)

            if isinstance(data, QuerySet):
                for obj in data:
                    dataset.append(self.export_resource(obj), fields, num_indent=self.NUM_INDENT, color=color)
            else:
                dataset.append(self.export_resource(data), fields, num_indent=self.NUM_INDENT, color=color)
...
class RecordResource(MultiModelResource):
    ...
    has_additional_offenses = ExcelField(attribute='has_additional_offenses', column_name='Additional Offenses Exist')
    jurisdiction = ExcelField(attribute='jurisdiction', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.JURISDICTION_MAP.values())}"', showDropDown=True))
    sex = ExcelField(attribute='sex', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.SEX_MAP.keys())}"', showDropDown=True))
    ...
    def get_export_fields(self):
        return [field for field in super().get_export_fields() if field.column_name not in ('batch_id',)]
    ...
    class Meta:
        ...
        exclude = ('id', 'batch', 'batch_file', 'date_uploaded', 'data',)
        export_order = ('batch_id','label','file_no','dob', 'jurisdiction', 'county', 'has_additional_offenses')
...
class OffenseResource(MultiModelResource):
    ...
    jurisdiction = ExcelField(attribute='jurisdiction', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.JURISDICTION_MAP.values())}"', showDropDown=True))
    plea = ExcelField(attribute='plea', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.VERDICT_CODE_MAP.keys())}"', showDropDown=True))
    verdict = ExcelField(attribute='verdict', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.VERDICT_CODE_MAP.keys())}"', showDropDown=True))
    disposition_method = ExcelField(attribute='disposition_method', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.DISPOSITION_METHOD_CODE_MAP.keys())}"', showDropDown=True))

    def get_export_fields(self):
        return [field for field in super().get_export_fields() if field.column_name != 'ciprs_record_id']
...
class OffenseRecordResource(MultiModelResource):
    ...
    severity = ExcelField(attribute='severity', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.SEVERITIES._db_values)}"', showDropDown=True))
    action = ExcelField(attribute='action', dropdown=DataValidation(type="list", formula1=f'"{",".join(constants.ACTIONS._db_values)}"', showDropDown=True))
...
class RecordSummaryResource(resources.ModelResource):
    ...
    def export(self, batch_object=None, *args, **kwargs):
        ...
        # Record
        for record_object in batch_object.records.order_by('file_no').prefetch_related('offenses','offenses__offense_records').all():
            ...
            for offense_object in record_object.offenses.all():
                ...
                # Offense Record
                self.offense_record_resource.export_excel(offense_object.offense_records.all(), dataset, color=color)
                dataset.append_separator()
                i+=1
